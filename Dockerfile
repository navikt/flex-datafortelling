...
FROM europe-north1-docker.pkg.dev/cgr-nav/pull-through/nav.no/python:3.14.5-dev AS builder
USER root
...
FROM europe-north1-docker.pkg.dev/cgr-nav/pull-through/nav.no/python:3.14.5-dev
USER root
